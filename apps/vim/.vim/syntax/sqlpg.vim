...
syn keyword pgsqlKeyword	 abort aggregate and as alias add
syn keyword pgsqlKeyword	 by before 
...
syn keyword pgsqlKeyword	 cast cascade character check column columns constraint 
syn keyword pgsqlKeyword	 database domain databases default distinct declare deallocate desc
syn keyword pgsqlKeyword	 deferrable deferred defaults do diagnostics
syn keyword pgsqlKeyword	 end exists exclusion found exception except each exit
...
syn keyword pgsqlKeyword	 index into immutable inner initially immediate inherits instead in if
...
syn keyword pgsqlKeyword	 no new null next
syn keyword pgsqlKeyword	 or operator outer order old on out open
syn keyword pgsqlKeyword	 plpgsql primary password primary privilege procedure partial prepared
syn keyword pgsqlKeyword	 partition preserve perform
syn keyword pgsqlKeyword	 role replace release reset references row rows rule restrict
...
syn keyword pgsqlKeyword	 to tranaction trigger table tables temp temporary tablespace type
syn keyword pgsqlKeyword	 unique using
syn keyword pgsqlKeyword	 verbose view values varying 
syn keyword pgsqlKeyword	 where with

" Statements
syn keyword pgsqlStatement       alter analyze comment create copy echo
syn keyword pgsqlStatement       delete drop execute explain grant insert lock noaudit
syn keyword pgsqlStatement       rename revoke savepoint select set truncate
syn keyword pgsqlStatement       prepare listen unlisten notify truncate update vacuum reindex 

" Transactions
syn keyword pgsqlTransaction     begin commit rollback
...
if version >= 508 || !exists("did_pgsql_syn_inits")
  ...
  HiLink pgsqlKeyword		 pgsqlSpecial
  HiLink pgsqlNumber		 Number
  HiLink pgsqlOperator		 pgsqlStatement
  ...
  HiLink pgsqlTransaction    pgsqlStatement
  HiLink pgsqlStatement      Statement
  HiLink psqlMeta            PreProd
